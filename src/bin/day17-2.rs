...
fn main() -> MainResult {
    ...
    let mut best_a_value = 0;
    let mut good_ending_a_values = vec![best_a_value];
    while let Some(fixed_c) = good_ending_a_values.pop() {
        for a in 0..8u64 {
            let a_value = (fixed_c << 3) | a;
            let mut runner =
                Runner::new(&[a_value, registers[1].1, registers[2].1], program.clone());
            runner.run();
            ...
            if output.len() <= program.len()
                && output
                    ...
                    .rev()
                    .enumerate()
                    .all(|(i, x)| *x as u8 == program[program.len() - i - 1])
            {
                best_a_value = a_value;
                good_ending_a_values.push(a_value)
            }
        }
    }
    println!("this? {best_a_value}");
    ...
    fn execute_instruction(&mut self) -> usize {
        ...
        match instruction {
            ...
            Instruction::Bxl => {
                ...
                let result = *b ^ operand as u64;
                // println!("bxl: {b} ^ {operand} = {result}");
                *b = result;
                self.ip + 2
            }
            Instruction::Bst => {
            ...
            Instruction::Out => {
                let output = self.as_combo(operand) & 0x07;
                // if output as u8 != self.program[self.output.len()] {
                //     // just end the program it's not worth it man
                //     self.program.len()
                // } else {
